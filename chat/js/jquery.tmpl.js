(function($){
    $.fn.extend({
        render: function( data, return_html ) {
            var html = $.render( this.attr('id') , this.html(), data );
    ...
    $.extend({
        render: function( name, tmpl, data ) {
            ...
            // Use a cached template, if available
            if ( $.templates[name] ) {
                fn = $.templates[name];
            } else {
                fn =  $.tmpl(tmpl);
        ...
        tmpl: function(str) {
            ...
            var fn = new Function("save","$i",
                "var $=jQuery,_=[];_.data=save;_.index=$i;" +
                ...
                .replace(/{(\/?)(\w+|.)(?:\((.*?)\))?(?: (.*?))?}/g, function(all, slash, type, fnargs, args) {
                    ...
                    if(args != undefined) args = args.replace(/\\'/g, "'");
